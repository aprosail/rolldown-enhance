...
vi.mock("node:process", () => ({ cwd: vi.fn(() => mockRoot) }))
...
describe("parsePathMappings", () => {
  test("undefined compilerOptions", () => expect(parsePaths()).toEqual({}))
  test("empty paths", () => expect(parsePaths({ paths: {} })).toEqual({}))
  test("with baseUrl", () => {
    const compilerOptions = {
      baseUrl: "./base",
      paths: { "@/*": ["./src/*"] },
    }
    expect(parsePaths(compilerOptions)).toEqual({ "@": `${mockRoot}/base/src` })
  })
})
